  ...
  <div class="app-container">
    ...
    <el-table-column label="操作" width="180" align="center" fixed="right">
      <template slot-scope="scope">
        ...
        <el-button type="danger" icon="el-icon-delete" size="mini" @click="removeDataById(scope.row.id)" title="删除" :disabled=" $hasBP('bnt.sysMenu.update')  === false && scope.row.children.length > 0" />
      </template>
    </el-table-column>
    </el-table>
...
export default {
  data(){
    return {
      sysMenuList: [],// 获取的菜单的列表,已经在后端构建好了树形结构
      dialogVisible: false,//控制修改或新增的页面的显示的
      sysMenu: defaultForm,// sysMenu对象
      // 下面的这几个暂时还不知道有什么用
      typeDisabled: false,
      type0Disabled: false,
      type1Disabled: false,
      ...
      expandKeys: [], // 暂时还不知道这个属性有什么用
      saveBtnDisabled: false, // 防止重复提交的按钮
  ...
  methods:{
    ...
    add(row){
      ...
      this.sysMenu = Object.assign({}, defaultForm) //对象合并
      this.sysMenu.id = '' // 为什么要把菜单的id搞成空字符串
      if(row) {
